...
# Create or update the location-analysis (Konum Analizi) virtual table and its weighted-heatmap style
# through GeoServer REST. Safe to rerun: existing resources are updated, missing resources are created.
#
# Differs from setup-poi.sh in one important way: NO bbox recalculation. vw_konum is empty under its
# fail-closed default (aid=-1), so recalculating would store a degenerate bbox — the featuretype XML
# declares Turkey's extent instead.
...
FEATURE_TYPE="vw_konum"
STYLE="vw_konum_heatmap"
...
if [[ ! -f "${FEATURE_XML}" || ! -f "${STYLE_SLD}" ]]; then
  echo "Konum feature-type or style artifact is missing under ${SCRIPT_DIR}." >&2
  exit 2
fi
...
# The fail-closed default (aid:-1) matches no run, so an EMPTY FeatureCollection is the expected,
# correct answer here — the point is that the SQL parses and the layer serves.
...
echo "Konum Analizi GeoServer provisioning complete and WFS verified."
