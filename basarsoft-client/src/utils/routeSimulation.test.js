...
test('simulation controls expose only the actions valid for the current run state', () => {
  ...
  const start = simulationControls({ simulation: { status: 'NotStarted' }, canControl: true, followed: false, route })
  ...
  const running = simulationControls({ simulation: { status: 'Running' }, canControl: true, followed: false, route })
  ...
  const stopped = simulationControls({ simulation: { status: 'Stopped' }, canControl: true, followed: true, route })
  assert.deepEqual(actionsOf(stopped), ['resume', 'end'])
  assert.equal(stopped.actions[0].label, 'Resume Simulation')

  const completed = simulationControls({ simulation: { status: 'Completed' }, canControl: true, followed: true, route })
  ...
  const viewer = simulationControls({ simulation: { status: 'Running' }, canControl: false, followed: false, route })
...
test('route readiness gates only a first start, never active-run controls', () => {
  ...
  const notStarted = simulationControls({
    simulation: { status: 'NotStarted' }, canControl: true, followed: false, route: invalidRoute,
  })
  assert.equal(notStarted.actions[0].disabled, true)

  const running = simulationControls({
    simulation: { status: 'Running' }, canControl: true, followed: false, route: invalidRoute,
  })
